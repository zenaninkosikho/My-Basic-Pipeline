...
const sslOptions = {
    ...
    cert: fs.readFileSync('./ssl/localhost.crt') // Path to your certificate file
};
...
app.post('/payment', verifyToken, async (req, res) => {
  try {
    ...
    // Check if customer exists (the `user.id` comes from the decoded token)
    const customer = await Customer.findById(req.user.id); // req.user.id comes from the verifyToken middleware
    if (!customer) {
      return res.status(404).json({ error: 'Customer not found' });
    }
...
app.post('/paymentverify', async (req, res) => {
  try {
    console.log('Received payment verify request:', req.body);  // Log the request body
    const { paymentId, customerId } = req.body;  // Extract paymentId and customerId from the request body

    // Ensure both paymentId and customerId are provided
    if (!paymentId || !customerId) {
      return res.status(400).json({ message: 'Missing paymentId or customerId' });
    }
    ...
    // Verify the customerId matches the customer associated with the payment
    if (payment.customerId.toString() !== customerId) {
      return res.status(400).json({ message: 'Customer ID does not match the payment information' });
    }

    // Retrieve the recipientAccount from the payment document
    const customerAccount = payment.recipientAccount;

    // Ensure necessary fields are available before creating the transaction
    if (!customerAccount) {
      return res.status(400).json({ message: 'Missing customer account information' });
    }

    // Prepare transaction data
    const transactionData = {
      customerId: payment.customerId,            // Include the customerId from payment
      customerAccount: customerAccount,          // Ensure customerAccount is available
    ...
    res.status(500).json({ message: 'Failed to verify payment - server' });
  }
});
